...
namespace Odin.Core.Transform
{
    ...
    public static class TransformEngine
    {
        ...
        private static bool EvaluateOr(string expr, ExecContext ctx)
        {
        ...
        private static bool EvaluateAnd(string expr, ExecContext ctx)
        {
        ...
        // NOT binds tightest; a leading whole-word `not` negates the primary that follows.
        private static bool EvaluateNot(string expr, ExecContext ctx)
        {
            string trimmed = expr.Trim();
            if (trimmed.Length >= 4
                && trimmed.Substring(0, 3).Equals("not", StringComparison.OrdinalIgnoreCase)
                && char.IsWhiteSpace(trimmed[3]))
            {
                return !EvaluateNot(trimmed.Substring(4), ctx);
            }
            return EvaluatePrimaryCondition(trimmed, ctx);
        }
        ...
        private static List<string> SplitTopLevel(string expr, string op)
        {
            ...
            for (int i = 0; i < expr.Length; i++)
            {
                ...
                if (i > 0 && i + op.Length < expr.Length
                    && char.IsWhiteSpace(expr[i - 1])
                    && char.IsWhiteSpace(expr[i + op.Length])
        ...
        private static bool EvaluatePrimaryCondition(string condition, ExecContext ctx)
        {
